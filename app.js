...
function back(){if(COMPOSITE_FLOW.has(current)){go(COMPOSITE_PREV[current]||'home');return}if(history.length>1)history.back();else go('home')}
...
function globalHotspots(){return topHotspotsFor(current).concat(bottomHotspots())}
...
[['scorecard',.085,.405],['composite',.355,.405],['maximo',.625,.405],['tempcomp',.085,.528],['sales',.355,.528],['labor-health',.625,.528]].forEach(([id,x,y])=>a.push(hs(titleOf(id),x,y,.27,.12,()=>go(id))));
